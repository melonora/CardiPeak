...
def averageFilter(y, filterWidth):
    """

    """
    filter = np.ones(filterWidth) / filterWidth
    averageY = np.array(y)
    halfMinusOne = int(filter.shape[0] / 2)
    for i in range(halfMinusOne, averageY.shape[0]-halfMinusOne):
        averageY[i] = (y[i-halfMinusOne:i+halfMinusOne+1] * filter).sum()
    return averageY
...
def obtainFrameValueLst(df):
    frames = df['Frame']
    values = df['Pixel value']
    return frames.tolist(), values.tolist()


def getMax(frames, values, threshold):
...
def startPeak(frames, values, derivative, threshold):
    if values[0] < threshold:
        below = True
    else:
        below = False
